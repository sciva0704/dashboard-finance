...
export default function SeccionCartera() {
  ...
  useEffect(() => {
    const cargarDatos = () => {
      try {
        const datosGuardados = localStorage.getItem("cartera_local");
        if (datosGuardados) {
          const { activosLocal, objetivoLocal } = JSON.parse(datosGuardados);
          if (activosLocal) setActivos(activosLocal);
  ...
  const guardarDatos = () => {
    try {
      const datos = {
        activosLocal: activos,
        objetivoLocal: objetivo,
      };
      localStorage.setItem("cartera_local", JSON.stringify(datos));
  ...
  const porcentajeProgreso =
    objetivo > 0 ? Math.min((inversionTotalAcumulada / objetivo) * 100, 100) : 0;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-slate-800 border border-slate-700 p-6 rounded-2xl shadow-xl">
          ...
          {!editandoMeta ? (
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => {
                  setInputMeta(objetivo.toLocaleString("es-AR"));
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button onClick={guardarMeta} className="text-emerald-400">
                ✔
              </button>
            </div>
          )}
      ...
      {/* Formulario */}
      <form
        onSubmit={agregarActivo}
        className="bg-slate-800 p-6 rounded-2xl border border-slate-700 mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 shadow-xl"
      >
        <select
          className="bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm outline-none focus:ring-2 focus:ring-emerald-500"
          value={categoria}
          onChange={(e) => {
        ...
        <div className="relative">
          <input
            list="activos-list"
            className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm outline-none focus:ring-2 focus:ring-emerald-500 disabled:opacity-30"
            placeholder="Ticker..."
            value={nombre}
        ...
        <input
          ...
          placeholder="Precio"
          step="any"
          className="bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm outline-none"
          value={precioCompra}
        ...
        />
        <button className="bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-black rounded-xl py-3 uppercase text-xs tracking-widest shadow-lg shadow-emerald-500/20 sm:col-span-2 lg:col-span-1">
          Cargar
        </button>
      </form>
            ...
            <tbody className="divide-y divide-slate-700">
              {activos.map((a) => (
                <tr key={a.id} className="hover:bg-slate-700/20 transition-colors">
                  <td className="p-5">
                    <span className="text-emerald-500 font-black text-lg block">
                      {a.nombre}
                    </span>
                  ...
                  <td className="p-5 text-right font-mono text-white">
                    ${a.precioCompra.toLocaleString("es-AR")}
                  </td>
                  <td className="p-5 text-right font-mono text-emerald-400 font-bold">
                    ${a.totalInvertido.toLocaleString("es-AR")}
                  </td>
